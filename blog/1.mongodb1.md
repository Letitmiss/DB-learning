...
3. mongdb支持单键索引,多键索引,属组索引,全文索引,地理位置索引
* 方便的冗余与扩展
1. 复制集保证数据安全
2. 分片扩展数据
* 良好的支持
...
```
* 以上操作时在suse系统上操作的,之后CentOS下载了最新版发现,必须要ymal格式的配置文件
...
local  0.000GB
test   0.000GB
MongoDB Enterprise > use gaocong  # use 直接指定一个不存在的数据库,会自动创建这个数据库
switched to db gaocong
MongoDB Enterprise > show dbs    # 但是show显示不出来,是因为还没有在这个数据库中插入数据
admin  0.000GB
local  0.000GB
test   0.000GB
MongoDB Enterprise > db
